...
def run_observation(data_dir: Path, output_path: Path) -> None:
	...
	subprocess.run(["docker", "run", "-v", f"{data_dir}:/data", IMAGE, "/scripts/generate_visibilities.sh", f"/data/{output_path.name}",],check=True)
...
def run_processing(data_dir: Path, visibilities_path: Path, output_prefix: Path) -> None:
	...
	subprocess.run(["docker", "run", "-v", f"{data_dir}:/data", IMAGE, "/scripts/process_visibilities.sh", f"/data/{visibilities_path.name}", f"/data/{output_prefix.name}"],check=True)
